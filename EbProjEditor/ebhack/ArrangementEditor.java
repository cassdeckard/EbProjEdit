 ...
 * the format described in {@see #makeArrangementNumber(int, int, boolean,
 * boolean)}.
 * 
 * @author AnyoneEB
 */
public abstract class ArrangementEditor extends AbstractButton
   ...
   * currently selected is most likely defined by a separate tile selector wiget.
   * This should return the tile number from that wiget.
   * 
  ...
  /**
   * Returns the actual size of each tile in pixels. It is assumed that all tiles
   * are squares. This should probably return 8, but your value may be different.
   * 
  ...
  /**
   * Returns the zoom factor. For example if this returns 2, the arrangement will
   * be drawn with twice the height and twice the width as actual size.
   * 
   * @return zoom factor
   */
   ...
   * tiles are actually drawn at accounting for zoom and (optionally) grid lines.
   * 
   * @param includeGrid if true, grid lines are included in the tile size. default
   *                    is true
  ...
  public int getDrawnTileSize(boolean includeGrid) {
    return (getTileSize() * getZoom()) + (isDrawGridLines() && includeGrid ? 1 : 0);
  }
  ...
  /**
   * Returns whether or not this arrangement is editable. If this returns false,
   * all clicks will jump to the clicked on tile using
  ...
  /**
   * Returns whether or not this should paint. If this returns false, this will
   * not attempt to paint. This is to prevent errors from the painting method
   * attempting to read from non-inited data or to prevent GUI errors from
   * painting.
   * 
  ...
  /**
   * Returns the current sub-palette. This is stored in the arrangement data when
   * a tile is placed.
   * 
  ...
  /**
   * Returns the arrangement data for the specified point. Note that point does
   * not mean it is measured in pixels. (0, 0) is the top-left tile. (1, 0) is the
   * next tile to the left. Important, note that this data in the arrangement
   * format as described in {@see #makeArrangementNumber(int, int, boolean,
   * boolean)}.
   * 
  ...
  /**
   * Returns the arrangement data for the entire arrangement. Important, note that
   * this data in the arrangement format as described in
  ...
  /**
   * Sets the arrangement data at the specified point. Note that point does not
   * mean it is measured in pixels. (0, 0) is the top-left tile. (1, 0) is the
   * next tile to the left. Important, note that this data needs to be set in the
   * arrangement format as described in {@see #makeArrangementNumber(int, int,
   * boolean, boolean)}.
   * 
   * @param x    x-coordinate of point
   * @param y    y-coordinate of point
  ...
  /**
   * Sets the arrangement data for the entire arrangement. Important, note that
   * this data needs to be set in the arrangement format as described in
   * {@see #makeArrangementNumber(int, int, boolean, boolean)}.
   * 
  ...
  /**
   * Returns an image of the current arrangement. Note that this may need to call
   * other methods for infomation on how to correctly draw like
   * {@link #getZoom()}and {@link #isDrawGridLines()}. The selection is an array
   * the same size as the arrangement. In it, a -1 indicates transparent, that is,
   * it means to draw whatever is in the arrangement at that point. Any number
   * greater than -1 is an arrangement number, and it should be drawn as one,
   * except selection areas should also be highlighted (translucent yellow is
   * normal, but any color could be used).
   * 
   * @param selection area in selection, -1's indicate transparent (no selection),
   *                  other numbers are arrangement numbers to be used instead of
   *                  the actual arrangement number at that location and to be
   *                  highlighted
   ...
   * @return <code>Image</code> of tile number <code>tile</code> in the current
   *         palette
   */
  protected abstract Image getTileImage(int tile, int subPal, boolean hFlip, boolean vFlip);
  ...
  /**
   * Describes the currently selected area. This is the same size of the entire
   * arrangement. Any values of -1 indicate no selection. Values above -1 are
   * arrangement values that indicate there is a selection at that point.
   */
   ...
   * actually write to number to anything, it just returns it. If you wish to edit
   * an arrangement which has a different format, override this. <br>
   * <br>
   ...
   */
  public short makeArrangementNumber(int tile, int subPalette, boolean hFlip, boolean vFlip) {
    return (short) ((tile & 0x03ff) | (((subPalette + 2) & 7) << 10) | (hFlip ? 0x4000 : 0) | (vFlip ? 0x8000 : 0));
  }
  ...
  private void leftClickAction(int x, int y) {
    ...
    if (isSelectionNull()) {
      if (isTile(x, y))
        setArrangementData(x, y, makeArrangementNumber(getCurrentTile(), getCurrentSubPalette(), false, false));
    } else {
      flattenSelection();
    }
    this.repaint();
  ...
  public void mouseClicked(MouseEvent me) {
    if ((me.getModifiers() & MouseEvent.ALT_MASK) != 0)
      return;
    addUndo();
    int x = me.getX() / (getTileSize() * getZoom() + (isDrawGridLines() ? 1 : 0));
    int y = me.getY() / (getTileSize() * getZoom() + (isDrawGridLines() ? 1 : 0));
  ...
  private short[][] createNewSelection(int nx, int ny) {
    ...
    for (int x = 0; x < getTilesWide(); x++)
      for (int y = 0; y < getTilesHigh(); y++)
        if (x + cx >= 0 && x + cx < getTilesWide() && y + cy >= 0 && y + cy < getTilesHigh())
        ...
        for (int dx = 0; dx < getTilesWide(); dx++)
          for (int dy = 0; dy < getTilesHigh(); dy++)
            if (newSel[dx][dy] != drawnSelection[dx][dy])
              drawTile(dx, dy, newSel[dx][dy] == -1 ? getArrangementData(dx, dy) : newSel[dx][dy],
                  newSel[dx][dy] != -1);
  ...
  private void drawTile(Graphics g, int x, int y, int arr, boolean highlight) {
    g.drawImage(getTileImage(getTileOfArr(arr), getSubPalOfArr(arr), isArrHFlip(arr), isArrVFlip(arr)),
        (x * 8 * getZoom()) + (isDrawGridLines() ? x : 0), (y * 8 * getZoom()) + (isDrawGridLines() ? y : 0),
        (8 * getZoom()), (8 * getZoom()), null);
    if (highlight) {
      g.setColor(new Color(255, 255, 0, 128));
      g.fillRect((x * 8 * getZoom()) + (isDrawGridLines() ? x : 0), (y * 8 * getZoom()) + (isDrawGridLines() ? y : 0),
          (8 * getZoom()), (8 * getZoom()));
    }
  }
  ...
  private void drawTile(Graphics g, int x, int y) {
    drawTile(g, x, y, selection[x][y] == -1 ? getArrangementData(x, y) : selection[x][y], selection[x][y] != -1);
  }
  ...
  public void repaintTile(int tile) {
    for (int x = 0; x < getTilesWide(); x++)
      for (int y = 0; y < getTilesHigh(); y++)
        if ((getTileOfArr(selection[x][y])) == tile || getTileOfArr(getArrangementData(x, y)) == tile)
          drawTile(x, y);
  }
  ...
  public void resetPreferredSize() {
    this.setPreferredSize(
        new Dimension((getTilesWide() * (getDrawnTileSize())) - 1, (getTilesHigh() * (getDrawnTileSize())) - 1));
  }
  ...
  /**
   * Merges the selection into the arrangement data. -1's are ignored, for other
   * values the arrangement data is set to what the selection is. Note that the
   * selection is cleared by this method.
   */
