...
public abstract class TileSelector extends AbstractButton implements MouseListener, MouseMotionListener {
  ...
  /**
   * Returns the number of tiles this allows selection from. This will probably be
   * {@link #getTilesWide()}*{@link #getTilesHigh()}, but not always. The first
   * tile will always be refered to as tile 0, and the last tile as the number
   * this returns minus 1.
   * 
  ...
  private void setCurrentTile(int x, int y) {
    setCurrentTile(((y / (getDrawnTileSize())) * getTilesWide()) + (x / (getDrawnTileSize())));
  }
  ...
  protected void drawTile(Graphics g, int tile) {
    if (tile >= 0 && tile < getTileCount()) {
      g.drawImage(getTileImage(tile), (tile % getTilesWide()) * getDrawnTileSize(),
          (tile / getTilesWide()) * getDrawnTileSize(), getDrawnTileSize(false), getDrawnTileSize(false), null);
    }
  }
  ...
  protected void highlightTile(Graphics g, int tile) {
    ...
    g.fillRect((tile % getTilesWide()) * getDrawnTileSize(), (tile / getTilesWide()) * getDrawnTileSize(),
  ...
  public void mouseDragged(MouseEvent me) {
    if (!(me.getX() < 0 || me.getY() < 0 || me.getX() > getTilesWide() * getDrawnTileSize() - 1
        || me.getY() > getTilesHigh() * getDrawnTileSize() - 1))
      setCurrentTile(me.getX(), me.getY());
  }
