...
public class IntArrDrawingArea extends ImageDrawingArea {
  ...
  /**
   * Creates a new IntArrDrawingArea with the given int[][], Toolset, and palette.
   * 
  ...
  /**
   * Creates a new IntArrDrawingArea with the given int[][], Toolset, palette, and
   * zoom.
   * 
   * @param img   An int[][] of color indexes in <code>pal</code>.
   * @param tools Toolset
   ...
   */
  public IntArrDrawingArea(int[][] img, Toolset tools, ColorPalette pal, float zoom) {
    this(img, tools, pal);
    this.setZoom(zoom);
  }
  ...
  protected void doTool(int x1, int y1, int x2, int y2) {
    ...
    int rx1 = Math.min(x1, x2), ry1 = Math.min(y1, y2), rx2 = Math.max(x1, x2), ry2 = Math.max(y1, y2),
        w = Math.min(x2 - x1, img.length - rx1), h = Math.min(y2 - y1, img[0].length - ry1),
        rw = Math.min(rx2 - rx1, img.length - rx1), rh = Math.min(ry2 - ry1, img[0].length - ry1);
    switch (tool) {
      ...
      case Toolset.TOOL_ROUND_RECTANGLE:
        ...
        switch (fill) {
          case Toolset.FILL_NONE:
            drawRoundedRect(rx1, ry1, rx2 - rx1, ry2 - ry1, curve, curve, c, false);
            break;
          case Toolset.FILL_OPQUE:
            drawRoundedRect(rx1, ry1, rx2 - rx1, ry2 - ry1, curve, curve, c, true);
            break;
          case Toolset.FILL_BACKGROUND:
            drawRoundedRect(rx1, ry1, rx2 - rx1, ry2 - ry1, curve, curve, 0, true);
            drawRoundedRect(rx1, ry1, rx2 - rx1, ry2 - ry1, curve, curve, c, false);
            break;
        }
        break;
   ...
   * </strong>). Unfilled draws a regular rectangle instead, ways to do it better
   * are welcome.
   * 
   ...
   */
  private void drawRoundedRect(int rx1, int ry1, int w, int h, int curvex, int curvey, int c, boolean fill) {
    if (fill) {
      RoundRectangle2D rr = new RoundRectangle2D.Double(rx1, ry1, w, h, curvex, curvey);
  ...
  private void drawOval(int rx1, int ry1, int w, int h, int c, boolean fill) {
    if (fill) {
      ...
      for (double t = 0; t < Math.PI * 2; t += tp) {
        drawPoint((int) Math.round(rx1 + xr + (xr * Math.cos(t))), (int) Math.round(ry1 + yr + (yr * Math.sin(t))), c);
      }
    }
  }
  ...
  public void flattenSelection() {
    ...
    for (int x = selection.x; x < selection.width + selection.x; x++) {
      for (int y = selection.y; y < selection.height + selection.y; y++) {
        try {
          drawPoint(x, y, this.selectImg[x - selection.x][y - selection.y]);
        } catch (ArrayIndexOutOfBoundsException e) {
        }
      }
    }
   ...
   * {@link #getNewImage(int[][], Rectangle)}if you only want part of the int[][]
   * in the image.
   * 
  ...
  public Image getImage(int[][] img) {
    BufferedImage out = new BufferedImage(img.length, img[0].length, BufferedImage.TYPE_4BYTE_ABGR_PRE);
    ...
    if (this.isEnabled()) {
      ...
      for (int x = 0; x < this.drawingWidth; x++) {
        for (int y = 0; y < this.drawingHeight; y++) {
          if (selection.width > 0 && selection.height > 0 && selection.contains(x, y)) {
            g.setColor(pal.getColorOf(selectImg[x - selection.x][y - selection.y]));
      ...
      // }
      /*
       * else { g .drawImage( this.getImage(), 0, 0, getZoomedXY(drawingWidth),
       * getZoomedXY(drawingHeight), //Color.BLACK, this);
       */
      // selection image
      // if (selection.width > 0 && selection.height > 0)
      // {
   ...
   * (non-Javadoc)
   * 
   * @see net.starmen.pkhack.ImageDrawingArea#setClipboard(net.starmen.pkhack.
   * ImageDrawingArea.Clipboard)
   */
   ...
   * 
   * @see
   * net.starmen.pkhack.ImageDrawingArea#getNewImage(java.awt.image.BufferedImage,
   * int, int, int, int)
   */
   ...
   * this.getPoint(x, y), newCol = pal .getSelectedColorIndex(); if (currentCol ==
   * newCol) { undo(); return; } this.drawPoint(x, y, newCol); for (int ax = -1;
   * ax < 2; ax++) for (int ay = -1; ay < 2; ay++) if (x + ax > -1 && y + ay > -1
   * && x + ax < this.getDrawingWidth() && y + ay < this.getDrawingHeight())
   * //make sure it's on the image if (this.getPoint(x + ax, y + ay) ==
   * currentCol) doPaintBucket(x + ax, y + ay); }
   */
  ...
  protected void doPaintBucket(int x, int y) {
    int currentCol = this.getPoint(x, y), newCol = pal.getSelectedColorIndex();
