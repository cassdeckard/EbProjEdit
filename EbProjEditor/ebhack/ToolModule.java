...
public abstract class ToolModule {
  ...
  public static File chooseFile(boolean save, final String extension, final String description, String prefStr,
    ...
    try {
      ...
      jfc.setFileFilter(new FileFilter() {
        public boolean accept(File f) {
          if (extension == null)
            return true;
          else if ((f.getAbsolutePath().toLowerCase().endsWith("." + extension) || f.isDirectory()) && f.exists()) {
            return true;
          } else
            return false;
        }
   ...
   * 
   * @param image   <code>byte[][]</code> of color numbers in <code>int[x][y] form
   * &#64;param palette
   ...
   * @return An <code>Image</code> drawn in the specified palette.
   */
  public static BufferedImage drawImage(byte[][] image, Color[] palette, boolean hFlip, boolean vFlip) {
    BufferedImage out = new BufferedImage(image.length, image[0].length, BufferedImage.TYPE_INT_ARGB);
    WritableRaster r = out.getRaster();
    for (int x = 0; x < image.length; x++) {
      for (int y = 0; y < image[0].length; y++) {
        int colIndex = image[hFlip ? image.length - x - 1 : x][vFlip ? image[0].length - y - 1 : y] & 0xff;
        Color c = palette[colIndex];
        r.setPixel(x, y, new int[] { c.getRed(), c.getGreen(), c.getBlue(), c.getAlpha() });
      }
    }
    return out;
  }
   ...
   * <code>palette(image[x][y])</code>.
   * 
   * @param image   <code>int[][]</code> of color numbers in <code>int[x][y] form
   * &#64;param palette
   *            <code>Color[]</code> for converting color numbers to colors
   * @return An <code>Image</code> drawn in the specified palette.
   */
   ...
   * @return An <code>Image</code> drawn in the specified palette.
   */
  public static BufferedImage drawImage(int[][] image, Color[] palette, boolean hFlip, boolean vFlip) {
    BufferedImage out = new BufferedImage(image.length, image[0].length, BufferedImage.TYPE_INT_ARGB);
    Graphics g = out.getGraphics();
    for (int x = 0; x < image.length; x++) {
      for (int y = 0; y < image[0].length; y++) {
        g.setColor(palette[image[hFlip ? image.length - x - 1 : x][vFlip ? image[0].length - y - 1 : y]]);
        g.drawLine(x, y, x, y);
      }
    }
    return out;
  ...
  public static JMenuItem createJMenuItem(String name, char m, String key, String ac, ActionListener al) {
  ...
  /**
   * Creates a new <code>JFrame</code> with "Apply Changes" and "Close" buttons.
   * The <code>JFrame</code>'s content pane has a <code>BorderLayout</code> with
   * the south used. In the south there are two buttons: an "Apply Changes" button
   * with the action command "apply" and a "Close" button with the action command
   * "close". Both <code>addActionListener(al)</code>.
   * 
   ...
   * <code>JTextField</code> uses a {@link net.starmen.pkhack.MaxLengthDocument}or
   * a {@link net.starmen.pkhack.NumericMaxLengthDocument}.
   * 
   * @param len         Maximum length for the <code>JTextField</code>
   * @param numbersOnly if true, a <code>NumericMaxLengthDocument</code> is used
   *                    to allow only numbers to be typed
   ...
   */
  public static JTextField createSizedJTextField(int len, boolean numbersOnly, boolean hex) {
    return new JTextField(
        numbersOnly ? new NumericMaxLengthDocument(len, hex ? "[^0-9a-fA-F]" : null) : new MaxLengthDocument(len), "",
        len);
  }
  ...
  /**
   * Creates a <code>JTextField</code> with a maximum length. This
   * <code>JTextField</code> uses a {@link net.starmen.pkhack.MaxLengthDocument}.
   * 
   * @param len Maximum length for the <code>JTextField</code>
   * @return A new <code>JTextField</code>, which limits the number of input
   *         characters to <code>len</code>
   ...
   * <code>pairComponents(new JLabel(label), comp, true)</code>, except this sets
   * {@link JLabel#setLabelFor(java.awt.Component)}to <code>comp</code>
   * 
   ...
   */
  public static JPanel getLabeledComponent(String label, JComponent comp, String tooltip) // useful for making layouts
  {
    /*
     * return pairComponents(new JLabel(), pairComponents(new JLabel(label), comp,
     * true, tooltip), false);
     */
   ...
   * @param isHorizontal If true west and east are used, if false north and south
   *                     are used.
   ...
   */
  public static JPanel pairComponents(JComponent comp1, JComponent comp2, boolean isHorizontal, boolean stretch,
    ...
    out.add(comp2, (stretch ? BorderLayout.CENTER : (isHorizontal ? BorderLayout.EAST : BorderLayout.SOUTH)));
    return out;
  }
   ...
   *      String)
   */
  public static JPanel pairComponents(JComponent comp1, JComponent comp2, boolean isHorizontal, String tooltip) {
    return pairComponents(comp1, comp2, isHorizontal, false, tooltip, tooltip);
  }
   ...
   *      String)
   */
  public static JPanel pairComponents(JComponent comp1, JComponent comp2, boolean isHorizontal) {
  ...
  protected static abstract class SimpleComboBoxModel extends DefaultComboBoxModel implements ListDataListener {
    ...
    public void contentsChanged(ListDataEvent lde) {
      this.fireContentsChanged(lde.getSource(), lde.getIndex0(), lde.getIndex1());
      this.notifyListDataListeners(
          new ListDataEvent(this, lde.getType(), lde.getIndex0() + offset, lde.getIndex1() + offset));
    }
  ...
  /**
   * Returns the input <code>String</code> with the number in []'s before it. If
   * <code>useHexNumbers</code> is true then the number is in hex, otherwise it's
   ...
   * @see #createJComboBoxFromArray(Object[])
   */
  public static String getNumberedString(String in, int num, boolean useHexNumbers) {
    return "[" + (useHexNumbers
        ? (Integer.toHexString(num).length() == 1 ? "0" + Integer.toHexString(num) : Integer.toHexString(num))
        : Integer.toString(num)) + "] " + in.trim();
  }

  /**
   * Returns the input <code>String</code> with the number in []'s before it. If
   * useHexNumbers pref is true then the number is in hex, otherwise it's in
   * decimal. If the hex number comes out to a single digit, a zero will be added
   * before it. This was written to be used for making lists for
   * <code>JComboBox</code> 's.
   * 
  ...
  public static int getNumberOfString(String in, boolean useHexNumbers) {
    try {
      return Integer.parseInt(new StringTokenizer(in, "[]", false).nextToken(), useHexNumbers ? 16 : 10);
  ...
  /**
   * Sets up a <code>JComboBox</code> from an <code>Object[]</code>. Removes all
   * items from the <code>JComboBox</code> first. Calls
   ...
   * @param useHexNumbers If true hex is used for the numbers, else decimal is
   *                      used.
   * @return <code>out<code> with all the items from <code>in</code> added to it.
   * @see #getNumberedString(String, int, boolean)
   */
  public static JComboBox createJComboBoxFromArray(Object[] in, JComboBox out, boolean useHexNumbers) {
    out.removeAllItems();
    for (int i = 0; i < in.length; i++) {
      if (in[i] == null) {
        ...
        out.addItem(getNumberedString(in[i].toString(), i, useHexNumbers));
      }
    }
    return out;
  }
   ...
   * items from the <code>JComboBox</code> first. Calls
   * {@link #getNumberedString(String, int, boolean)}before adding each item
   * unless the item starts with "[". The boolean value is the useHexNumbers pref.
   * 
  ...
  /**
   * Creates and sets up a <code>JComboBox</code> from an <code>Object[]</code>.
   * Calls {@link #getNumberedString(String, int, boolean)}before adding each item
   * unless the item starts with "[". The boolean value is the
   * <code>useHexNumbers<code>
   * parameter passed to this method.
   * 
   ...
   * @param useHexNumbers If true hex is used for the numbers, else decimal is
   *                      used.
   * @return A new <code>JComboBox</code> with all the items from <code>in</code>
   *         added to it.
   * @see #getNumberedString(String, int, boolean)
   * @see #createJComboBoxFromArray(Object[], JComboBox, boolean)
   */
  public static JComboBox createJComboBoxFromArray(Object[] in, boolean useHexNumbers) {
   ...
   * then the item is selected on the JComboBox and true is returned. The search
   * starts at the item after the currently selected item or the first item if the
   * last item is currently selected. If the search <code>String</code> is not
   * found, then an error dialog pops-up and the function returns false.
   * 
   * @param text           <code>String</code> to seach for. Can be anywhere
   *                       inside any item.
   ...
   */
  public static boolean search(String text, JComboBox selector, boolean beginFromStart, boolean displayError) {
    if (text == null || selector == null || selector.getItemCount() == 0)
      return false;
    text = text.toLowerCase();
    for (int i = (selector.getSelectedIndex() + 1 != selector.getItemCount() && !beginFromStart
        ? selector.getSelectedIndex() + 1
        : 0); i < selector.getItemCount(); i++) {
    ...
    if (beginFromStart) {
      if (displayError)
        JOptionPane.showMessageDialog(null, "Sorry, your search was not found.", "Not found!",
   ...
   * @return Wether the search <code>String</code> is found.
   */
  public static boolean search(String text, JComboBox selector, boolean beginFromStart) {
   ...
   * found, then an error dialog pops-up and the function returns false.
   * 
   * @param text     <code>String</code> to seach for. Can be anywhere inside any
   *                 item.
   * @param selector <code>JComboBox</code> to get list from, and set if search is
   *                 found.
   * @return Wether the search <code>String</code> is found.
   */
  ...
  /**
   * Adds zeros to the start of a <code>String</code> up to <code>maxLen</code>.
   * This for making hex values look nice. If <code>in</code> is already as long,
   * or longer, than <code>maxLen</code> <code>in</code> will be returned.
   * 
   * @param in <code>String</code> to add zeros to
   * @return A new <code>String</code> with length >=<code>maxLen</code> by adding
   *         "0"'s to the start of <code>in</code>
   */
   ...
   * checked before reading the default file. First <code>rompath.filename</code>
   * will be searched for a ROM-specific file. If there is no ROM specific file,
   * the default file at net/starmen/pkhack/ <code>filename</code> will be used.
   * The array should be in the format <code>EntryNum - Entry\n</code>. All
   * whitespace is ignored (spaces are allowed within
   * <code>Entry</code>.<code>EntryNum</code> will be read as decimal or
   * hexidecimal depending on <code>hexNum</code>.
   * 
   ...
   * @param hexNum   if true entry numbers in the file will be interpreted as hex
   * @param out      array data will be read into, this same array will be
   *                 returned
   ...
   */
  public static void readArray(ClassLoader cl, String baseDir, String filename, String romPath, boolean hexNum,
      String[] out, int size) {
    if (out == null)
      out = new String[size];
    ...
    try {
      ...
      try {
        ...
        list = new CommentedLineNumberReader(new FileReader((romPath + "." + filename))).readUsedLines();
      } catch (Exception e1) {
         ...
         */
        list = new CommentedLineNumberReader(new InputStreamReader(cl.getResourceAsStream(baseDir + filename)))
            .readUsedLines();
      }
      String[] tempStr;
      for (int i = 0; i < list.length; i++) {
        if (list[i].substring(0, Math.min(7, list[i].length())).matches("[^#]*-[^#]*.*")) {
          tempStr = list[i].split("-", 2);
          int num = Integer.parseInt(tempStr[0].trim(), (hexNum ? 16 : 10));
  ...
  /**
   * Returns the base directory most files are located at. Ends with a separator.
   */
   ...
   * <code>toString()</code> method will be called on all elements of the array,
   * so only <code>String</code>'s will be seen as elements. This calls
   * {@link #addDataListener(Object[], ListDataListener)}so that
   * {@link #notifyDataListeners(Object[], ListDataEvent)}can be used to notify it
   * of updates. If <code>zeroBased</code> is true, the array values will be
   * pushed up by one (to offsets <code>1</code> to <code>array.length</code>).
   * Offset <code>0</code> will be set to <code>zeroString</code>.
   * 
   * @param array      <code>Object</code>'s whose <code>.toString()</code> method
   *                   will give the <code>String</code> to use for the
   *                   corresponding position in the <code>ComboBoxModel</code>.
   * @param zeroBased  If false, the values of <code>array</code> are pushed up
   *                   one and <code>zeroString</code> is used for the zero value.
   ...
   */
  public static SimpleComboBoxModel createComboBoxModel(final Object[] array, final boolean zeroBased,
   ...
   * {@link #addDataListener(Object[], ListDataListener)}so that
   * {@link #notifyDataListeners(Object[], ListDataEvent)}can be used to notify it
   * of updates. If <code>zeroString</code> is not null, the array values will be
   * pushed up by one (to offsets <code>1</code> to <code>array.length</code>).
   * Offset <code>0</code> will be set to <code>zeroString</code>.
   * 
   * @param array      <code>Object</code>'s whose <code>.toString()</code> method
   *                   will give the <code>String</code> to use for the
   *                   corresponding position in the <code>ComboBoxModel</code>.
   * @param zeroString <code>null</code> if array should start at element zero,
   ...
   */
  public static SimpleComboBoxModel createComboBoxModel(final Object[] array, final String zeroString) {
  ...
  /**
   * Retrives the <code>List</code> containing the <code>ListDataListener</code>'s
   * for the specified array. If it does not exist, it will be created and added
   * to {@link #comboBoxListeners}. Since this is an object, modifying (by adding
   * elements to) the returned value will modify the value in
   * <code>comboBoxListeners</code>.
   * 
  ...
  /**
   * Adds a <code>ListDataListener</code> to <code>array</code>. This should be
   * called by a <code>ComboBoxModel</code> representing <code>array</code> so it
   * can be notified of changes to <code>array</code> when
  ...
  /**
   * Creates a <code>JComboBox</code> showing the elements of <code>array</code>
   * as numbered <code>String</code>s. Since this adds a
   * <code>ListDataListener</code> using
   * {@link HackModule#addDataListener(Object[], ListDataListener)}, the
   * {@link #notifyDataListeners(Object[], ListDataEvent)}methods can be used to
   * force it to be redrawn. If <code>zeroBased</code> is false, the array values
   ...
   */
  public static JComboBox createComboBox(Object[] array, boolean zeroBased, String zeroString,
      final ActionListener al) {
    SimpleComboBoxModel model = createComboBoxModel(array, zeroBased, zeroString);
   ...
   * force it to be redrawn. If <code>zeroBased</code> is false, the array values
   * will be pushed up by one (to offsets <code>1</code> to
   * <code>array.length</code>). Offset <code>0</code> will be set to "Nothing".
   * If not null, <code>al</code> will be added as an <code>ActionListener</code>
   * to the return value.
   * 
   * @param array     <code>Object</code>'s whose <code>.toString()</code> method
   *                  will give the <code>String</code> to use for the
   *                  corresponding position in the <code>JComboBox</code>.
   * @param zeroBased If false, the values of <code>array</code> are pushed up one
   ...
   * @see #createComboBox(Object[])
   */
  public static JComboBox createComboBox(Object[] array, boolean zeroBased, final ActionListener al) {
   ...
   * {@link HackModule#addDataListener(Object[], ListDataListener)}, the
   * {@link #notifyDataListeners(Object[], ListDataEvent)}methods can be used to
   * force it to be redrawn. If <code>zeroString</code> is not null, the array
   ...
   *                   will give the <code>String</code> to use for the
   *                   corresponding position in the <code>JComboBox</code>.
   * @param zeroString <code>String</code> to use as element zero if it is not
   *                   null.
   * @param al         If not null, <code>JComboBox.addActionListener(al);</code>
   *                   will be called.
   * @return a <code>JComboBox</code> displaying <code>array</code> as
   ...
   * @see #createComboBox(Object[])
   */
  public static JComboBox createComboBox(Object[] array, String zeroString, final ActionListener al) {
   ...
   * {@link HackModule#addDataListener(Object[], ListDataListener)}, the
   * {@link #notifyDataListeners(Object[], ListDataEvent)}methods can be used to
   * force it to be redrawn. If not null, <code>al</code> will be added as an
   * <code>ActionListener</code> to the return value.
   * 
   * @param array <code>Object</code>'s whose <code>.toString()</code> method will
   *              give the <code>String</code> to use for the corresponding
   *              position in the <code>JComboBox</code>.
   * @param al    If not null, <code>JComboBox.addActionListener(al);</code> will
   *              be called.
   * @return a <code>JComboBox</code> displaying <code>array</code> as
   ...
   * @see #createComboBox(Object[])
   */
  public static JComboBox createComboBox(Object[] array, final ActionListener al) {
   ...
   * @see #writeArray(String, boolean, String[])
   */
  private static void readArray(String baseDir, String filename, String romPath, boolean hexNum, String[] out,
      int size) {
    readArray(ToolModule.class.getClassLoader(), baseDir, filename, romPath, hexNum, out, size);
  }

  /**
   * Reads a file into an array. See
   * {@link #readArray(String, String, String, boolean, String[], int)}for more
   * information.
   * 
   * @param filename where to look for the file. (rompath.filename or
   *                 /net/starmen/pkhack/filename will be used)
   ...
   * @see #writeArray(String, boolean, String[])
   */
  public static void readArray(ClassLoader cl, String baseDir, String filename, boolean hexNum, String[] out) {
   ...
   * @see #writeArray(String, boolean, String[])
   */
  private static void readArray(String baseDir, String filename, boolean hexNum, String[] out) {
  ...
  public void readArray(String filename, boolean hexNum, String[] out) {
    readArray(this.getClass().getClassLoader(), getDefaultBaseDir(), filename, null, hexNum, out, out.length);
  }

  /**
   * Reads a file into an array. Creates a new array with length <code>size</code>
   * to read data into. See
   * {@link #readArray(String, String, String, boolean, String[], int)}for more
   * information.
   * 
   * @param filename where to look for the file. (rompath.filename or
   ...
   * @see #writeArray(String, boolean, String[])
   */
  private static String[] readArray(String baseDir, String filename, String romPath, boolean hexNum, int size) {
   ...
   * @see #writeArray(String, boolean, String[])
   */
  private static String[] readArray(String baseDir, String filename, boolean hexNum, int size) {
    String[] out = new String[size];
   ...
   *                 <code>getDefaultBaseDir()</code> +<code>filename</code> will
   *                 be used)
   * @param hexNum   if true entry numbers in the file will be interpreted as hex
   * @param size     size of array to create to read data into
   * @return <code>String[size]</code> containing information from file
  ...
  public String[] readArray(String filename, boolean hexNum, int size) {
    String[] out = new String[size];
    readArray(this.getClass().getClassLoader(), getDefaultBaseDir(), filename, null, hexNum, out, size);
    return out;
  }

  /**
   * Creates a standard edit menu. Menu includes undo if requested and cut, copy.
   * paste, and delete. Action commands are "undo" "cut" "copy" "paste" "delete".
   * 
   * @param al          <code>ActionListener</code> to send
   *                    <code>ActionEvents</code> to
   * @param includeUndo if true an undo menu item is included, if false it is not
   *                    included
   * @return a edit menu
   */
  public static JMenu createEditMenu(ActionListener al, boolean includeUndo) {
    ...
    if (includeUndo) {
      editMenu.add(ToolModule.createJMenuItem("Undo", 'u', "ctrl Z", "undo", al));
  ...
  /**
   * Returns an integer representing how different two colors are. Zero indicates
   * the same color. 195075 would be the difference between black and white. The
   * calculation is to sum the squares of the differences of the color components
   * (red, green, and blue).
   * 
   * @param a a Color
   * @param b another Color, order does not matter
   * @return a number representing how different the two input colors are; higher
   *         means more different
   */
  ...
  /**
   * Returns in the color in a palette which is the least different from a color.
   * {@link #getColorDiff(Color, Color)}is used to determine how different two
   * colors are.
   * 
  ...
  /**
   * Converts an <code>Image</code> into a <code>int[][]</code> using the given
   * palette. For each pixel of the image, the corresponding element,
   * <code>out[x][y]</code> will be set to the index of the closest color in the
   * palette.
   * 
   * @param in  image to convert. Any data outside the size of <code>out</code>
   *            will be discarded.
   * @param out <code>int[][]</code> to write palette indexed image into. This
  ...
  public static void convertImage(Image in, int[][] out, Color[] pal) {
    ...
    for (int i = 0; i < w; i++) {
      for (int j = 0; j < h; j++) {
        out[i][j] = getNearestColorIndex(pal, new Color(pixels[(j * w) + i] & 0xf8f8f8));
      }
    }
  }
