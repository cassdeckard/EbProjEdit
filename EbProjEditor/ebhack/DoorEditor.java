...
public class DoorEditor extends ToolModule implements ActionListener, DocumentListener {
  ...
  private final String[] climbDirections = { "nw", "ne", "sw", "se", "nowhere" };
  private final String[] typeNames = { "switch", "rope", "ladder", "door", "escalator", "stairway", "object",
      "person" };
  ...
  public void init() {
    ...
    doorInfo.add(ToolModule.pairComponents(ToolModule.pairComponents(
        ToolModule.getLabeledComponent("Destination X:", ToolModule.pairComponents(destX, destXhint, false)),
        ToolModule.getLabeledComponent("Destination Y:", ToolModule.pairComponents(destY, destYhint, false)), true,
        true), ToolModule.pairComponents(gotoButton, seekButton, true, true), true, true));
  ...
  private void updateGUI() {
    ...
    if (door == null) {
      ...
      typeBox.setEnabled(true);
      if ((typeBox.getSelectedIndex() == 1) || (typeBox.getSelectedIndex() == 2)) {
        // Rope/Ladder
        dirClimbBox.setEnabled(false);
        dirBox.setEnabled(false);
        destX.setEnabled(false);
        ...
        flagField.setEnabled(false);
        textPtr.setEnabled(false);
      } else if ((typeBox.getSelectedIndex() == 4) || (typeBox.getSelectedIndex() == 5)) {
        // Stairs/escalator
      ...
      } else if ((typeBox.getSelectedIndex() == 6) || (typeBox.getSelectedIndex() == 7)) {
        // Object / Person
        dirClimbBox.setEnabled(false);
        dirBox.setEnabled(false);
        destX.setEnabled(false);
  ...
  public void actionPerformed(ActionEvent ae) {
    if (ae.getActionCommand().equals("apply")) {
      ...
      ebhack.Ebhack.main.showModule(MapEditor.class,
          new int[] { Integer.parseInt(destX.getText()) * 8, Integer.parseInt(destY.getText()) * 8 });
  ...
  public void changedUpdate(DocumentEvent e) {
    if ((destX.getText().length() > 0) && (destY.getText().length() > 0)) {
      destXhint.setText("%4 = " + (Integer.parseInt(destX.getText()) % 4));
      destYhint.setText("%4 = " + (Integer.parseInt(destY.getText()) % 4));
    }
  }
