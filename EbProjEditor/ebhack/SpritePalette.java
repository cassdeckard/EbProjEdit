 ...
 */
public class SpritePalette extends AbstractButton implements ColorPalette, MouseListener, MouseMotionListener {
  ...
  /**
   * Constructor for SpritePalette. Assumes 16 colors, square size of 20, and 2
   * rows.
   */
  ...
  public void changeSize(int size, int rows, int numCol) {
    ...
    this.setPreferredSize(new Dimension((squareSize * (cols + 2)) + 1, (squareSize * rows) + 1));
    repaint();
  }
   ...
   * unpredictable for an array with an odd number of colors. The array index of
   * each color will be it's color number. Two of the same color in the array may
   * mess stuff up.
   * 
  ...
  public void setPalette(Color[] c) {
    ...
    this.pal = c;
    this.setPreferredSize(new Dimension((squareSize * ((pal.length / rows) + 2)) + 1, (squareSize * rows) + 1));
  ...
  public void mouseClicked(MouseEvent me) {
    ...
    if (me.getX() < 0 || me.getX() > squareSize * cols || me.getY() < 0 || me.getY() > squareSize * rows)
      return;
    ...
    if (x < (squareSize * cols)) {
      ...
      if (newCol < pal.length) {
        ...
        if (editable
          ...
          Color cc = JColorChooser.showDialog(this, "Select a new color", this.getSelectedColor());
          if (cc == null)
            return;
          this.newColor = new Color(cc.getRed() & 0xf8, cc.getGreen() & 0xf8, cc.getBlue() & 0xf8);
  ...
  public void paint(Graphics g) {
    ...
    g.fillRect(0, 0, ((pal.length / rows) * squareSize) + 1, (rows * squareSize) + 1);
    if (pal == null) {
      g.setColor(Color.BLACK);
      for (int i = 0; i < (pal.length / rows); i++) {
        g.fillRect(i * squareSize, 0, squareSize - 1, squareSize - 1);
        g.fillRect(i * squareSize, squareSize, squareSize - 1, squareSize - 1);
      }
    } else {
      for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
          g.setColor(this.getColorOf(c + r * cols));
          g.fillRect((c * squareSize) + 1, (r * squareSize) + 1, squareSize - 1, squareSize - 1);
    ...
    g.setColor(col);
    g.fillRect((int) ((cols + 0.5) * squareSize), (int) (((rows - 1.0) / 2.0) * squareSize), squareSize, squareSize);
    g.setColor(Color.BLACK);
    g.drawRect((int) ((cols + 0.5) * squareSize), (int) (((rows - 1.0) / 2.0) * squareSize), squareSize, squareSize);
  }
  ...
  /**
   * Returns the newColor. The red, green, and blue values will each be theValue &
   * 0xe0 because SNES colors only have 3-bit precision (for each: red, green, and
   * blue) and this will be used as in a SNES palette.
   * 
   * @return Color
   */
   ...
   * 
   * @param editable If false, the user may not change the colors on the palette.
   */
