...
public class ImageDrawingArea extends DrawingArea {
  ...
  public interface ColorPalette extends Palette {
    ...
    /**
     * Sets the selected Color to the specified Color. If the specified Color is not
     * a valid selection, unpredictable results will follow.
     * 
     * @param c Color
     */
   ...
   * <code>imgDrawAreaA.setClipboard(imgDrawAreaB.getClipboard())</code> to make
   * imgDrawAreaA and imgDrawAreaB use the same clipboard.
   * 
   * @see ImageDrawingArea.Clipboard
   ...
   * @see ImageDrawingArea.ColorPalette
   */
  public ImageDrawingArea(Image img, Toolset tools, ColorPalette pal, float zoom) {
    super();
  ...
  /**
   * Changes the image and zoom to the specified image and zoom. Resets undo and
   * selection, but leaves clipboard.
   * 
   * @param img  new image
   * @param zoom new zoom
   */
   ...
   * zoomed. Important note: this draws the point zoomed. Meaning it will draw a
   * filled rectangle of the size <code>zoom</code> x <code>zoom</code> with the
   * top-left corner at x*zoom, y*zoom.
   * 
  ...
  public void paint(Graphics g) {
    if (this.isEnabled()) {
      ...
      if (this.drawingWidth * this.drawingHeight < 500) {
        ...
        g.drawImage(img, 0, 0, getZoomedXY(drawingWidth), getZoomedXY(drawingHeight),
            // Color.BLACK,
            this);
      }
      // selection image
      ...
      if (this.isDrawGridlines()) {
        ...
        for (int x = 0; x < this.drawingWidth; x++) {
          g.drawLine(getZoomedXY(x), 0, getZoomedXY(x), getZoomedXY(this.drawingHeight) - 1);
        }
        for (int y = 0; y < this.drawingHeight; y++) {
          g.drawLine(0, getZoomedXY(y), getZoomedXY(this.drawingWidth) - 1, getZoomedXY(y));
        }
        g.drawRect(0, 0, getZoomedXY(drawingWidth), getZoomedXY(drawingHeight));
      }
      // selection border
      if (selection.width > 0 && selection.height > 0) {
        // flattenSelection(g, true);
        ...
        // this);
        g.setColor(Color.WHITE);
        g.drawRect(getZoomedXY(selection.x), getZoomedXY(selection.y), getZoomedXY(selection.width),
            getZoomedXY(selection.height));
      }
    } else {
  ...
  public void doHFlip() {
    ...
    PixelGrabber pg = new PixelGrabber(img.getScaledInstance(w, h, 0), 0, 0, w, h, pixels, 0, w);
    try {
      pg.grabPixels();
    } catch (InterruptedException e) {
  ...
  private void doPaintBucketReg(int x, int y, Color newCol, Color currentCol) throws InterruptedException {
    ...
    if (x > -1 && y > -1 && x < this.getDrawingWidth() && y < this.getDrawingHeight()
  ...
  protected void doPaintBucket(int x, int y) throws InterruptedException {
    Color currentCol = this.getPointColor(x, y), newCol = pal.getSelectedColor();
  ...
  /**
   * Takes the start and end of drag and responds to them based on the selected
   * tool.
   * 
  ...
  protected void doTool(int x1, int y1, int x2, int y2) throws InterruptedException {
    ...
    switch (tool) {
      ...
      case Toolset.TOOL_OVAL:
        switch (fill) {
          /*
           * case Toolset.FILL_NONE : drawOval(rx1, ry1, rx2 - rx1, ry2 - ry1, c, false);
           * break; case Toolset.FILL_OPQUE : drawOval(rx1, ry1, rx2 - rx1, ry2 - ry1, c,
           * true); break; case Toolset.FILL_BACKGROUND : drawOval(rx1, ry1, rx2 - rx1,
           * ry2 - ry1, 0, true); drawOval(rx1, ry1, rx2 - rx1, ry2 - ry1, c, false);
           */
          case Toolset.FILL_NONE:
      ...
      case Toolset.TOOL_ROUND_RECTANGLE:
        ...
        switch (fill) {
          case Toolset.FILL_NONE:
            g.drawRoundRect(rx1, ry1, rx2 - rx1, ry2 - ry1, curve, curve);
            break;
          case Toolset.FILL_OPQUE:
            g.fillRoundRect(rx1, ry1, rx2 - rx1, ry2 - ry1, curve, curve);
            break;
          case Toolset.FILL_BACKGROUND:
            g.setColor(pal.getColorOf(0));
            g.fillRoundRect(rx1, ry1, rx2 - rx1, ry2 - ry1, curve, curve);
      ...
      case Toolset.TOOL_SELECTION:
        if (!isMakingSelect) {
          ...
          if (rx1 >= 0 && ry1 >= 0 && rx2 < getWidth() && ry2 < getHeight()) {
  ...
  public void mouseDragged(MouseEvent me) {
    if (this.isEnabled()) {
      ...
      if (!(tool == Toolset.TOOL_EYEDROPER || tool == Toolset.TOOL_PAINT_BUCKET || tool == Toolset.TOOL_PENCIL)
  ...
  public void mousePressed(MouseEvent me) {
    if (this.isEnabled()) {
      ...
      if (tools.getSelectedDrawingTool() == Toolset.TOOL_SELECTION) {
        isMakingSelect = ((!(selection.width > 0 && selection.height > 0)) || !(selection.contains(sx, sy)));
      }
  ...
  /**
   * Notifies ActionListener's that a change has been made. Run whenever a change
   * is made.
   */
  ...
  /**
   * Copies a specified part of an Image into a new Image. The new Image will be a
   * completely separate object from the orginal.
   * 
   ...
   */
  public BufferedImage getNewImage(BufferedImage img, int x, int y, int w, int h) {
    int[] pixels = new int[w * h];
    BufferedImage out = (BufferedImage) this.createImage(w, h);
    PixelGrabber pg = new PixelGrabber(img.getSubimage(x, y, w, h), 0, 0, w, h, pixels, 0, w);
    try {
      pg.grabPixels();
    } catch (InterruptedException e) {
      System.err.println("interrupted waiting for pixels!");
  ...
  public BufferedImage getNewImage(BufferedImage img, Rectangle coords) {
    return this.getNewImage(img, coords.x, coords.y, coords.width, coords.height);
  }
  ...
  /**
   * Copies a Image into a new Image. The new Image will be a completely separate
   * object from the orginal.
   * 
   * @param img Image to copy
   * @return A new Image containing the same information.
   * @see #getNewImage(BufferedImage, int, int, int, int)
  ...
  /**
   * Writes the selection with the specified Graphics. Does not actually remove
   * the selection.
   * 
  ...
  public void flattenSelection(Graphics g, boolean isZoomed) {
    ...
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        ...
        g.setColor(c);
        if (isZoomed) {
          g.fillRect(getZoomedXY(i + x), getZoomedXY(j + y), getZoomedXY(1), getZoomedXY(1));
        } else {
  ...
  /**
   * Writes the selection to the image. Does not actually remove the selection.
   */
  ...
  /**
   * Pastes a given selection. It stays as a selection until the user flattens it.
   * 
  ...
  /**
   * Pastes a given selection. If flatten is false, it stays as a selection until
   * the user flattens it.
   * 
   * @param img     Image to paste, must fit exactly in sel
   * @param sel     Selection rectangle to place image in
