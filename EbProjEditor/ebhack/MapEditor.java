...
public class MapEditor extends ToolModule
    implements ActionListener, DocumentListener, AdjustmentListener, MouseWheelListener, ComponentListener {
  private JTextField xField, yField, itemField, townMapXField, townMapYField;
  private JComboBox tilesetChooser, palChooser, musicChooser, townMapChooser, settingChooser, townMapImageChooser,
      townMapArrowChooser;
  ...
  private JMenuItem /* findSprite, */ copySector, pasteSector, copySector2, pasteSector2, undo, redo;
  ...
  public void init() {
    ...
    menu = new JMenu("Edit");
    undo = ToolModule.createJMenuItem("Undo Tile Change", 'u', "control Z", "undoMap", this);
    undo.setEnabled(false);
    menu.add(undo);
    redo = ToolModule.createJMenuItem("Redo Tile Change", 'r', "control Y", "redoMap", this);
    redo.setEnabled(false);
    menu.add(redo);
    menu.add(new JSeparator());
    copySector = ToolModule.createJMenuItem("Copy Sector", 'c', "control C", "copySector", this);
    menu.add(copySector);
    pasteSector = ToolModule.createJMenuItem("Paste Sector", 'p', "control V", "pasteSector", this);
    menu.add(pasteSector);
    copySector2 = ToolModule.createJMenuItem("Copy Sector Attributes", ' ', "control shift C", "copySector2", this);
    menu.add(copySector2);
    pasteSector2 = ToolModule.createJMenuItem("Paste Sector Attributes", ' ', "control shift V", "pasteSector2", this);
    menu.add(pasteSector2);
    menu.add(new JSeparator());
    menu.add(ToolModule.createJMenuItem("Clear Map", 'm', null, "delAllMap", this));
    menu.add(ToolModule.createJMenuItem("Delete All Sprites", 's', null, "delAllSprites", this));
    menu.add(ToolModule.createJMenuItem("Delete All Doors", 'o', null, "delAllDoors", this));
    menu.add(ToolModule.createJMenuItem("Clear Enemy Placements", 'e', null, "delAllEnemies", this));
    menu.add(ToolModule.createJMenuItem("Delete/Clear All Of The Above", 'a', null, "delAllEverything", this));
    menu.add(new JSeparator());
    menu.add(ToolModule.createJMenuItem("Clear Tile Image Cache", 't', "control R", "resetTileImages", this));
    menuBar.add(menu);
    ...
    menu.add(checkBox);
    menu.add(new JSeparator());
    checkBox = new PrefsCheckBox("Show Sector Attributes", prefs, "showSectorAttrs", false, 'c');
    checkBox.setActionCommand("showSectorAttrs");
    checkBox.addActionListener(this);
    menu.add(checkBox);
    checkBox = new PrefsCheckBox("Show Sector Town Map Settings", prefs, "showSectorAttrs2", false, 'c');
    checkBox.setActionCommand("showSectorAttrs2");
    checkBox.addActionListener(this);
    menu.add(checkBox);
    checkBox = new PrefsCheckBox("Show Coordinates", prefs, "showCoords", false, 'c');
    ...
    checkBox.addActionListener(this);
    // menu.add(checkBox);
    menu.add(new PrefsCheckBox("Mask Overscan in Preview", prefs, "maskOverscan", false, 'o'));
    menuBar.add(menu);

    menu = new JMenu("Tools");
    menu.add(ToolModule.createJMenuItem("Export as Image", 'i', "control i", "exportAsImage", this));
    menuBar.add(menu);
    ...
    for (int i = 0; i < MapData.Sector.TOWN_MAP_NAMES.length; ++i) {
      townMapChooser.addItem(getNumberedString(ToolModule.capitalize(MapData.Sector.TOWN_MAP_NAMES[i]), i));
    }
    ...
    for (int i = 0; i < MapData.Sector.SETTING_NAMES.length; ++i)
      settingChooser.addItem(getNumberedString(ToolModule.capitalize(MapData.Sector.SETTING_NAMES[i]), i));
    ...
    for (int i = 0; i < MapData.Sector.TOWN_MAP_IMAGES.length; ++i) {
      townMapImageChooser.addItem(getNumberedString(ToolModule.capitalize(MapData.Sector.TOWN_MAP_IMAGES[i]), i));
    }
    ...
    for (int i = 0; i < MapData.Sector.TOWN_MAP_ARROWS.length; ++i) {
      townMapArrowChooser.addItem(getNumberedString(ToolModule.capitalize(MapData.Sector.TOWN_MAP_ARROWS[i]), i));
    }
    ...
    mapDisplay = new MapDisplay(map, copySector, pasteSector, copySector2, pasteSector2, undo, redo, pixelCoordLabel,
        warpCoordLabel, tileCoordLabel, prefs);
    ...
    mainWindow.getContentPane()
        .add(ToolModule.pairComponents(ToolModule.pairComponents(tileSelector, tileSelector.getScrollBar(), false),
  ...
  private void loadTilesetNames() {
    ...
    for (int i = 0; i < MapData.NUM_MAP_TSETS; i++)
      tilesetChooser.addItem(getNumberedString(TileEditor.TILESET_NAMES[TileEditor.getDrawTilesetNumber(i)], i, false));
    tilesetChooser.addActionListener(this);
  }
  ...
  public void show(Object o) {
    ...
    if (o instanceof DoorEditor) {
      ...
      int[] coords = (int[]) o;
      mapDisplay.setMapXY(coords[0] / MapData.TILE_WIDTH, coords[1] / MapData.TILE_HEIGHT);
      updateXYFields();
      updateXYScrollBars();
      mapDisplay.repaint();
    }
  ...
  public static class MapDisplay extends AbstractButton implements ActionListener, MouseListener, MouseMotionListener {
    ...
    private MapData map;
    private JMenuItem copySector, pasteSector, copySector2, pasteSector2, undoButton, redoButton;
    ...
    private JMenuItem detailsDoor, delDoor, cutDoor, copyDoor, editDoor, jumpDoor;
    ...
    public MapDisplay(MapData map, JMenuItem copySector, JMenuItem pasteSector, JMenuItem copySector2,
        JMenuItem pasteSector2, JMenuItem undoButton, JMenuItem redoButton, JLabel pixelCoordLabel,
        JLabel warpCoordLabel, JLabel tileCoordLabel, YMLPreferences prefs) {
      super();
      ...
      spritePopupMenu.add(detailsNPC = ToolModule.createJMenuItem("Sprite @ ", ' ', null, null, this));
      detailsNPC.setEnabled(false);
      spritePopupMenu.add(ToolModule.createJMenuItem("New NPC", 'n', null, "newNPC", this));
      spritePopupMenu.add(delNPC = ToolModule.createJMenuItem("Delete NPC", 'd', null, "delNPC", this));
      spritePopupMenu.add(cutNPC = ToolModule.createJMenuItem("Cut NPC", 'c', null, "cutNPC", this));
      spritePopupMenu.add(copyNPC = ToolModule.createJMenuItem("Copy NPC", 'y', null, "copyNPC", this));
      spritePopupMenu.add(ToolModule.createJMenuItem("Paste NPC", 'p', null, "pasteNPC", this));
      spritePopupMenu.add(switchNPC = ToolModule.createJMenuItem("Switch NPC", 's', null, "switchNPC", this));
      spritePopupMenu.add(moveNPC = ToolModule.createJMenuItem("Move NPC", 'm', null, "moveNPC", this));

      // Create Door popup menu
      doorPopupMenu = new JPopupMenu();
      doorPopupMenu.add(detailsDoor = ToolModule.createJMenuItem("Door @ ", ' ', null, null, this));
      detailsDoor.setEnabled(false);
      doorPopupMenu.add(ToolModule.createJMenuItem("New Door", 'n', null, "newDoor", this));
      doorPopupMenu.add(delDoor = ToolModule.createJMenuItem("Delete Door", 'd', null, "delDoor", this));
      doorPopupMenu.add(cutDoor = ToolModule.createJMenuItem("Cut Door", 'c', null, "cutDoor", this));
      doorPopupMenu.add(copyDoor = ToolModule.createJMenuItem("Copy Door", 'y', null, "copyDoor", this));
      doorPopupMenu.add(ToolModule.createJMenuItem("Paste Door", 'p', null, "pasteDoor", this));
      doorPopupMenu.add(editDoor = ToolModule.createJMenuItem("Edit Door", 'e', null, "editDoor", this));
      doorPopupMenu.add(jumpDoor = ToolModule.createJMenuItem("Jump to Destination", 'j', null, "jumpDoor", this));

      addMouseListener(this);
      addMouseMotionListener(this);

      setPreferredSize(new Dimension(screenWidth * MapData.TILE_WIDTH + 2, screenHeight * MapData.TILE_HEIGHT + 2));
    }
    ...
    public void paintComponent(Graphics g) {
      ...
      if (isEnabled())
        ...
        g2d.draw(
            new Rectangle2D.Double(0, 0, screenWidth * MapData.TILE_WIDTH + 2, screenHeight * MapData.TILE_HEIGHT + 2));
      }
    }
    ...
    private void drawMap(Graphics2D g) {
      ...
      for (i = 0; i < screenHeight; i++) {
        for (j = 0; j < screenWidth; j++) {
          ...
          pal = TileEditor.tilesets[TileEditor.getDrawTilesetNumber(sector.tileset)].getPaletteNum(sector.tileset,
              sector.palette);
          g.drawImage(getTileImage(TileEditor.getDrawTilesetNumber(sector.tileset), map.getMapTile(x + j, y + i), pal),
              j * MapData.TILE_WIDTH + 1, i * MapData.TILE_HEIGHT + 1, MapData.TILE_WIDTH, MapData.TILE_HEIGHT, this);
          if (drawTileNums && !gamePreview) {
            drawNumber(g, map.getMapTile(x + j, y + i), j * MapData.TILE_WIDTH + 1, i * MapData.TILE_HEIGHT + 1, false,
                false);
          }
        }
      }
      ...
      if (editMap && (selectedSector != null)) {
        int sXt, sYt;
        if (((sXt = sectorX * MapData.SECTOR_WIDTH) + MapData.SECTOR_WIDTH >= x) && (sXt < x + screenWidth)
            && ((sYt = sectorY * MapData.SECTOR_HEIGHT) + MapData.SECTOR_HEIGHT >= y) && (sYt < y + screenHeight)) {
          g.setPaint(Color.yellow);
          g.draw(new Rectangle2D.Double((sXt - x) * MapData.TILE_WIDTH + 1, (sYt - y) * MapData.TILE_HEIGHT + 1,
      ...
      if (drawSprites) {
            ...
            try {
              ...
              for (MapData.SpriteEntry e : area) {
                ...
                if (spriteBoxes && !gamePreview) {
                  ...
                  g.draw(new Rectangle2D.Double(e.x + (j - x) * MapData.TILE_WIDTH - wh[0] / 2,
                      e.y + (i - y) * MapData.TILE_HEIGHT - wh[1] + 8, wh[0] + 1, wh[1] + 1));
                }
                g.drawImage(map.getSpriteImage(npc.sprite, npc.direction),
                    e.x + (j - x) * MapData.TILE_WIDTH - wh[0] / 2 + 1, e.y + (i - y) * MapData.TILE_HEIGHT - wh[1] + 9,
                    this);
                if (drawSpriteNums && !gamePreview) {
                  drawNumber(g, e.npcID, e.x + (j - x) * MapData.TILE_WIDTH - wh[0] / 2,
                      e.y + (i - y) * MapData.TILE_HEIGHT - wh[1] + 8, false, true);
                }
              }
            } catch (Exception e) {
        ...
        if (editSprites && (movingNPC != -1)) {
          if (spriteBoxes) {
            g.setPaint(Color.RED);
            g.draw(new Rectangle2D.Double(movingDrawX - 1, movingDrawY - 1, movingNPCdim[0] + 1, movingNPCdim[1] + 1));
          }
      ...
      if (drawDoors) {
          ...
          for (j = x & (~7); j < (x & (~7)) + screenWidth + 8; j += 8) {
            try {
              ...
              for (MapData.Door e : area) {
                ...
                g.draw(new Rectangle2D.Double(e.x * 8 + (j - x) * MapData.TILE_WIDTH + 1,
                    e.y * 8 + (i - y) * MapData.TILE_HEIGHT + 1, 8, 8));
                g.draw(new Rectangle2D.Double(e.x * 8 + (j - x) * MapData.TILE_WIDTH + 3,
                    e.y * 8 + (i - y) * MapData.TILE_HEIGHT + 3, 4, 4));
                g.setPaint(Color.WHITE);
                g.draw(new Rectangle2D.Double(e.x * 8 + (j - x) * MapData.TILE_WIDTH + 2,
                    e.y * 8 + (i - y) * MapData.TILE_HEIGHT + 2, 6, 6));
                g.draw(new Rectangle2D.Double(e.x * 8 + (j - x) * MapData.TILE_WIDTH + 4,
                    e.y * 8 + (i - y) * MapData.TILE_HEIGHT + 4, 2, 2));
              }
            } catch (Exception e) {
        ...
        if (editDoors && (movingDoor != null)) {
          ...
          g.draw(new Rectangle2D.Double(movingDrawX + 1, movingDrawY + 1, 8, 8));
          g.draw(new Rectangle2D.Double(movingDrawX + 3, movingDrawY + 3, 4, 4));
          g.setPaint(Color.WHITE);
          g.draw(new Rectangle2D.Double(movingDrawX + 2, movingDrawY + 2, 6, 6));
          g.draw(new Rectangle2D.Double(movingDrawX + 4, movingDrawY + 4, 2, 2));
        }

        if (seekDoor) {
          g.setPaint(Color.WHITE);
          g.draw(new Rectangle2D.Double(seekDrawX + 1, seekDrawY + 1, 8, 8));
          g.draw(new Rectangle2D.Double(seekDrawX + 3, seekDrawY + 3, 4, 4));

          g.setPaint(new Color(57, 106, 177));
          g.draw(new Rectangle2D.Double(seekDrawX + 2, seekDrawY + 2, 6, 6));
          g.draw(new Rectangle2D.Double(seekDrawX + 4, seekDrawY + 4, 2, 2));
        }
      }
      ...
      if (drawEnemies) {
        ...
        for (i = -(y % 2); i < screenHeight; i += 2) {
          for (j = -(x % 2); j < screenWidth; j += 2) {
            // Draw the grid
            Rectangle2D rect = new Rectangle2D.Double(j * MapData.TILE_WIDTH + 1, i * MapData.TILE_HEIGHT + 1,
            ...
            if (a != 0) {
              ...
              drawNumber(g, a, j * MapData.TILE_WIDTH + 1, i * MapData.TILE_HEIGHT + 1, false, false);
            }
          }
        }
      }
      ...
      if (drawHotspots) {
        ...
        for (i = 0; i < map.numHotspots(); ++i) {
          ...
          if (((tx1 >= 0) && (tx1 <= screenWidth) && (ty1 >= 0) && (ty1 <= screenHeight))
              || ((tx2 >= 0) && (tx2 <= screenWidth) && (ty2 >= 0) && (ty2 <= screenHeight))) {
            g.setPaint(Color.PINK);
            g.fill(new Rectangle2D.Double(hs.x1 * 8 - x * MapData.TILE_WIDTH + 1,
                hs.y1 * 8 - y * MapData.TILE_HEIGHT + 1, (hs.x2 - hs.x1) * 8, (hs.y2 - hs.y1) * 8));

            drawNumber(g, i, hs.x1 * 8 - x * MapData.TILE_WIDTH + 1, hs.y1 * 8 - y * MapData.TILE_HEIGHT + 1, false,
                false);
          }
        }
        ...
        if (editHotspots && (editHS != null)) {
          ...
          if (editHSx1 != -1) {
            ...
            g.fill(new Rectangle2D.Double(tx1, ty1, hsMouseX - tx1, hsMouseY - ty1));
          } else {
            g.fill(new Rectangle2D.Double(hsMouseX + 1, hsMouseY + 1, 65, 65));
          }
        }
      }
      ...
      if (gamePreview && tvPreview) {
        g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1.0F));
        g.setColor(Color.BLACK);
        g.fillRect(0, 0, tvPreviewX - tvPreviewW, screenHeight * MapData.TILE_HEIGHT);
        g.fillRect(tvPreviewX + tvPreviewW, 0, (screenWidth * MapData.TILE_WIDTH) - tvPreviewX - tvPreviewW,
            screenHeight * MapData.TILE_HEIGHT);
        g.fillRect(0, 0, screenWidth * MapData.TILE_WIDTH, tvPreviewY - tvPreviewH);
        g.fillRect(0, tvPreviewY + tvPreviewH, screenWidth * MapData.TILE_WIDTH,
            (screenHeight * MapData.TILE_HEIGHT) - tvPreviewY - tvPreviewH);

        // hardcoded for sprite of size 16,24
        g.drawImage(map.getSpriteImage(1, 2), tvPreviewX - 7, tvPreviewY - 15, this);
      }
    }

    private Rectangle2D textBG;

    private void drawNumber(Graphics2D g, int n, int x, int y, boolean hex, boolean above) {
      String s;
      if (hex)
    ...
    private void drawGrid(Graphics2D g) {
      ...
      g.drawLine(screenWidth * MapData.TILE_WIDTH + 1, screenHeight * MapData.TILE_HEIGHT + 1,
          screenWidth * MapData.TILE_WIDTH + 1, screenHeight * MapData.TILE_HEIGHT + 1);
    }
    ...
    public static Image getTileImage(int loadtset, int loadtile, int loadpalette) {
      if (tileImageCache[loadtset][loadtile][loadpalette] == null) {
        try {
          tileImageCache[loadtset][loadtile][loadpalette] = TileEditor.tilesets[loadtset].getArrangementImage(loadtile,
              loadpalette);
        } catch (IndexOutOfBoundsException ioobe) {
          System.err.println("Invalid tset/tile/pal: " + loadtset + "/" + loadtile + "/" + loadpalette);
    ...
    public void mouseClicked(MouseEvent e) {
      ...
      if ((e.getX() >= 1) && (e.getX() <= screenWidth * MapData.TILE_WIDTH + 2) && (e.getY() >= 1)
        ...
        if (editMap) {
          ...
          if (e.getButton() == MouseEvent.BUTTON3) {
            ...
            if (popupSE == null) {
              ...
              final int areaX = ((x + popupX / MapData.TILE_WIDTH) / 8) * MapData.TILE_WIDTH * 8;
              final int areaY = ((y + popupY / MapData.TILE_HEIGHT) / 8) * MapData.TILE_HEIGHT * 8;
              detailsNPC.setText("Sprite @ (" + (areaX + popupSE.x) + "," + (areaY + popupSE.y) + ")");
          ...
          if (e.getButton() == MouseEvent.BUTTON3) {
            ...
            if (popupDoor == null) {
              ...
              final int areaX = ((x + popupX / MapData.TILE_WIDTH) / MapData.SECTOR_WIDTH) * MapData.SECTOR_WIDTH
              ...
              detailsDoor.setText(ToolModule.capitalize(popupDoor.type) + " @ (" + (areaX + popupDoor.x) + ","
                  + (areaY + popupDoor.y) + ")");
          ...
          doorSeeker.seek(x * 4 + seekDrawX / 8, y * 4 + seekDrawY / 8);
          doorSeeker = null;
          ...
          int mx = ((e.getX() - 1) / 8) + (x * 4), my = ((e.getY() - 1) / 8) + (y * 4);
          if (editHS != null) {
            if (editHSx1 == -1) {
              editHSx1 = mx;
              editHSy1 = my;
    ...
    public void actionPerformed(ActionEvent ae) {
      if (ae.getActionCommand().equals("newNPC")) {
        ...
        String input = JOptionPane.showInputDialog(this, "Switch this to a different NPC", popupSE.npcID);
        if (input != null) {
        ...
        String input = JOptionPane.showInputDialog(this, "New X in pixels", areaX * MapData.TILE_WIDTH * 8 + popupSE.x);
        if (input == null)
          return;
        newSpriteX = Integer.parseInt(input);

        input = JOptionPane.showInputDialog(this, "New Y in pixels", areaY * MapData.TILE_HEIGHT * 8 + popupSE.y);
        if (input == null)
          return;
        ...
        ebhack.Ebhack.main.showModule(MapEditor.class, new int[] { popupDoor.destX * 8, popupDoor.destY * 8 });
      }
    }

    // Sprites
    private MapData.SpriteEntry getSpriteEntryFromMouseXY(int mouseX, int mouseY) {
      int areaX = (x + mouseX / MapData.TILE_WIDTH) / 8, areaY = (y + mouseY / MapData.TILE_HEIGHT) / 8;
    ...
    private void updateCoordLabels(final int mouseX, final int mouseY) {
      if ((mouseX >= 0) && (mouseY >= 0)) {
        pixelCoordLabel.setText(
            "Pixel X,Y: (" + (x * MapData.TILE_WIDTH + mouseX - 1) + "," + (y * MapData.TILE_WIDTH + mouseY - 1) + ")");
        warpCoordLabel.setText("Warp X,Y: (" + (x * 4 + (mouseX - 1) / 8) + "," + (y * 4 + (mouseY - 1) / 8) + ")");
        tileCoordLabel.setText("Tile X,Y: (" + (x + (mouseX - 1) / MapData.TILE_WIDTH) + ","
    ...
    public void pasteSector(Sector copiedSector, int sectorX2, int sectorY2, int[][] copiedSectorTiles) {
      for (int i = 0; i < copiedSectorTiles.length; i++)
        for (int j = 0; j < copiedSectorTiles[i].length; j++) {
          map.setMapTile(sectorX * 8 + j, sectorY * 4 + i, copiedSectorTiles[i][j]);
        }
  ...
  private class TileSelector extends AbstractButton implements MouseListener, AdjustmentListener {
    ...
    public TileSelector(int width, int height) {
      ...
      setPreferredSize(new Dimension(width * MapData.TILE_WIDTH + 3, height * MapData.TILE_HEIGHT + 3));
    ...
    public void selectTile(int tile) {
      this.tile = tile;
      if ((tile < scroll.getValue() * height) || (tile > (scroll.getValue() + width + 1) * height))
      ...
      for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
          ...
          if (dtile < 203) {
            ...
            g.fill(new Rectangle2D.Double(i * MapData.TILE_WIDTH + 1, j * MapData.TILE_HEIGHT + 1, MapData.TILE_WIDTH,
                MapData.TILE_HEIGHT));

            g.setPaint(Color.black);
            message = addZeros(Integer.toString(dtile), 3);
            rect = g.getFontMetrics().getStringBounds(message, g);
            rect.setRect(i * MapData.TILE_WIDTH + 1, j * MapData.TILE_HEIGHT + 1, rect.getWidth(), rect.getHeight());
            g.fill(rect);
            g.setPaint(Color.white);
            g.drawString(message, (float) (i * MapData.TILE_WIDTH + 1),
                (float) (j * MapData.TILE_HEIGHT + rect.getHeight()));
            if (dtile == tile) {
              g.setPaint(Color.yellow);
              g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.6F));
              g.fillRect(i * MapData.TILE_WIDTH + 1, j * MapData.TILE_HEIGHT + 1, MapData.TILE_WIDTH,
                  MapData.TILE_HEIGHT);
              g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1.0F));
            }
          }
    ...
    private void drawTiles(Graphics2D g) {
      ...
      for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
          ...
          if (dtile < 1024) {
            g.drawImage(
                MapDisplay.getTileImage(TileEditor.getDrawTilesetNumber(tilesetChooser.getSelectedIndex()), dtile,
                    mapDisplay.getSelectedSectorPalNumber()),
                i * MapData.TILE_WIDTH + 1, j * MapData.TILE_HEIGHT + 1, MapData.TILE_WIDTH, MapData.TILE_HEIGHT, this);
            if (dtile == tile) {
              g.setPaint(Color.yellow);
              g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.6F));
              g.fillRect(i * MapData.TILE_WIDTH + 1, j * MapData.TILE_HEIGHT + 1, MapData.TILE_WIDTH,
    ...
    private void drawGrid(Graphics2D g) {
      ...
      // Blank pixel in the bottom right corner
      g.drawLine(width * MapData.TILE_WIDTH + 1, height * MapData.TILE_HEIGHT + 1, width * MapData.TILE_WIDTH + 1,
          height * MapData.TILE_HEIGHT + 1);

      // Draw border
      g.setColor(Color.black);
      g.draw(new Rectangle2D.Double(0, 0, width * MapData.TILE_WIDTH + 2, height * MapData.TILE_HEIGHT + 2));
    }
    ...
    public void mouseClicked(MouseEvent e) {
      if ((e.getButton() == MouseEvent.BUTTON1) && isEnabled()) {
        ...
        repaint();
        if (e.isShiftDown()) {
          ebhack.Ebhack.main.showModule(TileEditor.class,
              new int[] { TileEditor.getDrawTilesetNumber(tilesetChooser.getSelectedIndex()),
                  mapDisplay.getSelectedSectorPalNumber(), tile });
        }
      }
    }
  ...
  public static class MapData {
    ...
    public void load(Project proj) {
      importMapTiles(proj.getFilename("eb.MapModule", "map_tiles"));
    ...
    public SpriteEntry getSpriteEntryFromCoords(int areaX, int areaY, int x, int y) {
      ...
      for (SpriteEntry e : spriteAreas[areaY][areaX]) {
        ...
        if ((e.x >= x - wh[0] / 2) && (e.x <= x + wh[0] / 2) && (e.y >= y - wh[1] / 2) && (e.y <= y + wh[1] / 2)) {
          return e;
        }
      }
      return null;
    ...
    public Door getDoorFromCoords(int areaX, int areaY, int x, int y) {
      for (Door e : doorAreas[areaY][areaX]) {
        if ((x <= e.x + 1) && (x >= e.x) && (y <= e.y + 1) && (y >= e.y)) {
          return e;
        }
      }
      return null;
    ...
    private void importNPCs(File f) {
      ...
      try {
        ...
        for (Map.Entry<Integer, Map<String, Object>> entry : sectorsMap.entrySet()) {
          npc = new NPC((Integer) entry.getValue().get("Sprite"), (String) entry.getValue().get("Direction"));
      ...
      for (int i = 0; i < spriteGroups.length; ++i) {
        ...
        try {
          BufferedImage sheet = ImageIO.read(
              new File(proj.getFilename("eb.SpriteGroupModule", "SpriteGroups/" + ToolModule.addZeros(i + "", 3))));
    ...
    public static class Door {
      ...
      private static final Color[] doorColors = new Color[] { new Color(83, 81, 84), // Switch
        ...
        for (Map.Entry<Integer, Map<Integer, List<Map<String, Object>>>> rowEntry : doorsMap.entrySet()) {
          ...
          for (Map.Entry<Integer, List<Map<String, Object>>> entry : rowEntry.getValue().entrySet()) {
            ...
            for (Map<String, Object> de : entry.getValue()) {
              Door d = new Door((Integer) de.get("X"), (Integer) de.get("Y"), ((String) de.get("Type")).toLowerCase());
        ...
        for (Map.Entry<Integer, Map<Integer, List<Map<String, Integer>>>> rowEntry : spritesMap.entrySet()) {
          ...
          for (Map.Entry<Integer, List<Map<String, Integer>>> entry : rowEntry.getValue().entrySet()) {
            ...
            for (Map<String, Integer> spe : entry.getValue()) {
              area.add(new SpriteEntry(spe.get("X"), spe.get("Y"), spe.get("NPC ID")));
            }
          }
        }
      } catch (FileNotFoundException e) {
    ...
    private void importSectors(File f) {
      ...
      try {
        ...
        for (Map.Entry<Integer, Map<String, Object>> entry : sectorsMap.entrySet()) {
          ...
          sec.townmapImage = (String) (entry.getValue().get("Town Map Image"));
          sec.townmapArrow = (String) (entry.getValue().get("Town Map Arrow"));
          sec.townmapX = (Integer) (entry.getValue().get("Town Map X"));
          sec.townmapY = (Integer) (entry.getValue().get("Town Map Y"));
        }
      } catch (FileNotFoundException e) {
        // TODO Auto-generated catch block
        e.printStackTrace();
    ...
    public static class Sector {
      public static final String[] TOWN_MAP_NAMES = new String[] { "none", "onett", "twoson", "threed", "fourside",
          "scaraba", "summers", "none 2" };
      public static final String[] SETTING_NAMES = new String[] { "none", "indoors", "exit mouse usable",
          "lost underworld sprites", "magicant sprites", "robot sprites", "butterflies", "indoors and butterflies" };
      public static final String[] TOWN_MAP_IMAGES = new String[] { "none", "onett", "twoson", "threed", "fourside",
          "scaraba", "summers" };
      public static final String[] TOWN_MAP_ARROWS = new String[] { "none", "up", "down", "right", "left" };

      public int tileset = 0, palette = 0, music = 0, item, townmapX, townmapY;
      public String townmap, setting, teleport, townmapImage, townmapArrow;
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getActionCommand().equals("sectorChanged")) {
      ...
      if (updatePaletteChooser(mapDisplay.getSelectedSector().tileset, mapDisplay.getSelectedSector().palette))
      ...
      mapDisplay.getSelectedSector().setTownMapNum(townMapChooser.getSelectedIndex());
    } else if (e.getSource().equals(settingChooser)) {
      mapDisplay.getSelectedSector().setSettingNum(settingChooser.getSelectedIndex());
    } else if (e.getSource().equals(teleportCheckbox)) {
      mapDisplay.getSelectedSector().setTeleportEnabled(teleportCheckbox.isSelected());
    } else if (e.getSource().equals(townMapImageChooser)) {
      mapDisplay.getSelectedSector().setTownMapImageNum(townMapImageChooser.getSelectedIndex());
    } else if (e.getSource().equals(townMapArrowChooser)) {
      mapDisplay.getSelectedSector().setTownMapArrowNum(townMapArrowChooser.getSelectedIndex());
      ...
      int sure = JOptionPane.showConfirmDialog(mainWindow, "Are you sure you want to clear the map and sector data?",
          "Are you sure?", JOptionPane.YES_NO_OPTION);
      if (sure == JOptionPane.YES_OPTION) {
        map.nullMapData();
        mapDisplay.repaint();
      ...
      int sure = JOptionPane.showConfirmDialog(mainWindow, "Are you sure you want to delete the door placement data?",
          "Are you sure?", JOptionPane.YES_NO_OPTION);
      if (sure == JOptionPane.YES_OPTION) {
        map.nullDoorData();
        mapDisplay.repaint();
      ...
      int sure = JOptionPane.showConfirmDialog(mainWindow, "Are you sure you want to clear the enemy data?",
          "Are you sure?", JOptionPane.YES_NO_OPTION);
      if (sure == JOptionPane.YES_OPTION) {
        map.nullEnemyData();
        mapDisplay.repaint();
      ...
      int sure = JOptionPane.showConfirmDialog(mainWindow, "Are you sure you want to all of the map data?",
          "Are you sure?", JOptionPane.YES_NO_OPTION);
      if (sure == JOptionPane.YES_OPTION) {
        map.nullMapData();
        map.nullSpriteData();
      ...
      String tpt = JOptionPane.showInputDialog(mainWindow, "Enter TPT entry to search for.", Integer.toHexString(0));
      int tptNum, yesno;
      try {
      ...
      /*
       * if (tpt != null) { for (int i = 0; i < (MapData.HEIGHT_IN_SECTORS / 2)
       * MapData.WIDTH_IN_SECTORS; i++) { ArrayList sprites = map.getSpritesData(i);
       * MapData.SpriteLocation spLoc; int areaY, areaX; for (int j = 0; j <
       * sprites.size(); j++) { spLoc = (MapData.SpriteLocation) sprites.get(j); if
       * (spLoc.getTpt() == tptNum) { areaY = i / MapEditor.widthInSectors; areaX = i
       * - (areaY * MapEditor.widthInSectors); gfxcontrol .setMapXY( (areaX *
       * MapEditor.sectorWidth) + (spLoc.getX() / MapEditor.tileWidth), (areaY *
       * MapEditor.sectorHeight * 2) + (spLoc.getY() / MapEditor.tileHeight)); yesno =
      ...
      for (int i = 0; i < copiedSectorTiles.length; i++)
        for (int j = 0; j < copiedSectorTiles[i].length; j++)
          copiedSectorTiles[i][j] = map.getMapTile(j + sectorX * 8, i + sectorY * 4);
      ...
      /*
       * } else if (ac.equals(ENEMY_SPRITES)) { gfxcontrol.toggleEnemySprites(); }
       * else if (ac.equals(ENEMY_COLORS)) { gfxcontrol.toggleEnemyColors(); } else if
       * (ac.equals(EVENTPAL)) { gfxcontrol.toggleEventPalette();
       */
    } else if (e.getActionCommand().equals("undoMap")) {
      if (!mapDisplay.undoMapAction()) {
        JOptionPane.showMessageDialog(mainWindow, "There are no actions to undo.", "Error", JOptionPane.ERROR_MESSAGE);
      }
    } else if (e.getActionCommand().equals("redoMap")) {
      if (!mapDisplay.redoMapAction()) {
        JOptionPane.showMessageDialog(mainWindow, "There are no actions to redo.", "Error", JOptionPane.ERROR_MESSAGE);
      }
    } else if (e.getActionCommand().equals("exportAsImage")) {
      final JTextField inputX = ToolModule.createSizedJTextField(Integer.toString(MapData.WIDTH_IN_TILES).length(),
          true);
      final JTextField inputX2 = ToolModule.createSizedJTextField(Integer.toString(MapData.WIDTH_IN_TILES).length(),
          true);
      final JTextField inputY = ToolModule.createSizedJTextField(Integer.toString(MapData.HEIGHT_IN_TILES).length(),
          true);
      final JTextField inputY2 = ToolModule.createSizedJTextField(Integer.toString(MapData.HEIGHT_IN_TILES).length(),
          true);
      final Object[] message = { "X1:", inputX, "Y1:", inputY, "X2:", inputX2, "Y2:", inputY2 };
      ...
      final BufferedImage bImg = new BufferedImage(imgW * MapData.TILE_WIDTH + 2, imgH * MapData.TILE_HEIGHT + 2,
      ...
      try {
        ...
        jfc.setFileFilter(new FileFilter() {
          public boolean accept(File f) {
            if ((f.getAbsolutePath().toLowerCase().endsWith(".png") || f.isDirectory()) && f.exists()) {
              return true;
            }
            return false;
          }
        ...
        if (jfc.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
          ...
          BufferedImage croppedImage = bImg.getSubimage(1, 1, imgW * MapData.TILE_WIDTH, imgH * MapData.TILE_HEIGHT);
  ...
  public void changedUpdate(DocumentEvent e) {
    if ((e.getDocument().equals(xField.getDocument()) || e.getDocument().equals(yField.getDocument()))
        && (yField.getText().length() > 0) && (xField.getText().length() > 0)) {
      int newX = Integer.parseInt(xField.getText()), newY = Integer.parseInt(yField.getText());
    ...
    } else if (e.getDocument().equals(itemField.getDocument()) && (itemField.getText().length() > 0)) {
      mapDisplay.getSelectedSector().item = Integer.parseInt(itemField.getText());
    } else if (e.getDocument().equals(townMapXField.getDocument()) && (townMapXField.getText().length() > 0)) {
      mapDisplay.getSelectedSector().townmapX = Integer.parseInt(townMapXField.getText());
    } else if (e.getDocument().equals(townMapYField.getDocument()) && (townMapYField.getText().length() > 0)) {
      mapDisplay.getSelectedSector().townmapY = Integer.parseInt(townMapYField.getText());
    }
  }
  ...
  public void mouseWheelMoved(MouseWheelEvent e) {
    if (e.isControlDown()) { // Horizontal scrolling
      setMapXY(mapDisplay.getMapX() + (e.getWheelRotation() * 4), mapDisplay.getMapY());
    } else { // Vertical scrolling
      setMapXY(mapDisplay.getMapX(), mapDisplay.getMapY() + (e.getWheelRotation() * 4));
    }
  }
