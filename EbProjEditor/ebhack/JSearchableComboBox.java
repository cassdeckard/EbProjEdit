...
public class JSearchableComboBox extends JComponent implements ActionListener, KeyListener {
   ...
   * @param jcb        The JComboBox this is a wrapper for.
   * @param text       Text to label the JComboBox with.
   * @param searchSize the size of the search text field if <code>smode</code> is
   *                   <code>SEARCH_LEFT</code> or <code>SEARCH_RIGHT</code>
   * @param smode      Search mode, one of {@link #SEARCH_NONE},
   *                   {@link #SEARCH_LEFT},{@link #SEARCH_RIGHT},
   *                   {@link #SEARCH_EDIT}.
   */
  public JSearchableComboBox(final JComboBox jcb, String text, int searchSize, int smode) {
    super();
    this.comboBox = jcb;
    if (smode == SEARCH_EDIT) {
      ...
      jcb.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent ae) {
          if (jcb.getSelectedIndex() == -1 && jcb.getSelectedItem() != null)
            ToolModule.search(jcb.getSelectedItem().toString(), jcb);
        }
      });
  ...
  private void initGraphics(int smode) {
    ...
    if (smode == SEARCH_EDIT || smode == SEARCH_NONE) {
      ...
      add(ToolModule.pairComponents(tf, findb, true), (smode == SEARCH_LEFT ? BorderLayout.WEST : BorderLayout.EAST));
      add(ToolModule.pairComponents(label, comboBox, true),
